...
attr_reader :ground, :peak_heights

def setup
  sketch_title 'Reflection2'
  @orb = Orb.new(50.0, 50.0, 3.0)
  @velocity = Vect.new(0.5, 0.0)
  # Initialize ground peak heights
  @peak_heights = (0..SEGS + 1).map do
    rand(height - 40.0..height - 30.0)
  end
  @ground = (0..SEGS).map do |i|
    Ground.new(width.to_f / SEGS * i,
               peak_heights[i],
               width.to_f / SEGS * (i + 1),
               peak_heights[i + 1])
  end
