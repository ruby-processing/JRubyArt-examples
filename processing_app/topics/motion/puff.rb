...
def draw
  ...
  (0...CELLS).each do |i|
    if i == 0
      ...
      if (px[i] >= width - cell_radius[i] / 2) || (px[i] <= cell_radius[i] / 2)
        radii_x[i] *= -1
        cell_radius[i] = rand(1..40)
        frequency[i] = rand(-13..13)
      end
  ...
  @speed_x *= -1 if (head_x >= width - cell_radius[0] / 2) || (head_x <= cell_radius[0] / 2)
  @speed_y *= -1 if  (head_y >= height - cell_radius[0] / 2) || (head_y <= cell_radius[0] / 2)
end
