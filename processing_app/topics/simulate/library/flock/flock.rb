...
class Boid
  ...
  def seek(target)
    desired = target - location  # A vector pointing from location to target
    # Normalize desired and scale to maximum speed
    desired.normalize!
    desired *= MAXSPEED
    # Steering = Desired minus Velocity
    steer = desired - velocity
    steer.set_mag(MAXFORCE) { steer.mag > MAXFORCE } # Limit to max
    steer
  end
  ...
  # Wraparound
  def borders
    location.x = width + sz * 0.5 if location.x < -sz * 0.5
    location.y = height + sz * 0.5 if location.y < -sz * 0.5
    location.x = -sz if location.x > width + sz * 0.5
    location.y = -sz * 0.5 if location.y > height + sz * 0.5
  end
  ...
  def separate(boids)
    ...
    boids.each do |other|
      ...
      next unless d > 0 && d < desiredseparation
  ...
  def cohesion(boids)
    neighbordist = 50
    sum = Vec2D.new   # Start with a zero vector to accumulate all locations
    count = 0
    boids.each do |other|
      d = location.dist(other.location)
      if d > 0 && d < neighbordist
