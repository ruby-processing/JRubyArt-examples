...
def set_ellipse(points, radius, control_radius)
  ...
  points.times do |i|
    px[i] = width / 2 + Math.cos(angle.radians) * radius
    py[i] = height / 2 + Math.sin(angle.radians) * radius
    cx[i] = width / 2 + Math.cos((angle + control_angle_1).radians) *
            control_radius / Math.cos((control_angle_1).radians)
    cy[i] = height / 2 + Math.sin((angle + control_angle_1).radians) *
            control_radius / Math.cos((control_angle_1).radians)
    cx2[i] = width / 2 + Math.cos((angle + control_angle_2).radians) *
             control_radius / Math.cos((control_angle_1).radians)
    cy2[i] = height / 2 + Math.sin((angle + control_angle_2).radians) *
             control_radius / Math.cos((control_angle_1).radians)
